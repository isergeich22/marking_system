...
const navMenu = document.querySelector('#mainNavbar')
if (burger) {
...
const tableHeader = document.querySelector('.marks-table-header')
const buttonTop = document.querySelector('#top')
const multipleList = document.querySelector('.multiple-list')
const statusList = document.querySelector('.status-list')
const statusRows = document.querySelectorAll('.col-status')

window.addEventListener('scroll', () => {
    if (scrollY > 100) {
        if (tableHeader) tableHeader.classList.add('--pinned')
        buttonTop.style.display = 'block'
    } else {
        if (tableHeader) tableHeader.classList.remove('--pinned')
...
if (window.location.href.indexOf('filter') < 0 && window.location.href.indexOf('yandex') < 0) {
    ...
    if (lastButton) {
        const orderInput = document.querySelectorAll('.col-order')
        const submitButton = document.querySelectorAll('.order-number')
        const marksSpan = document.querySelectorAll('.col-mark')
