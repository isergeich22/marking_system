...
router.get('/ozon', async function(req, res){
    ...
    res.render('import-products', { title: 'Импорт - OZON', items, marksOrderUrl: '/ozon_marks_order', buttons })

})

router.get('/ozon_marks_order', async function(req, res){
...
router.get('/ozon/:from', async function(req, res){
    ...
    let response = await axios.post('https://api-seller.ozon.ru/v4/posting/fbs/list', {
        ...
        'dir': 'asc',
        'filter': {
            'since':`${req.params.from}:30:00.000Z`,
            "statuses": [
                "awaiting_packaging",
                "awaiting_deliver"
            ],
    ...
    const items = names.map(el => ({ name: el.name, isNew: new_items.indexOf(el.name) >= 0 }))
    res.render('import-products', { title: 'Импорт - OZON', items, marksOrderUrl: `/ozon_marks_order/${req.params.from}`, buttons })

})

router.get('/ozon_marks_order/:from', async function(req, res){
